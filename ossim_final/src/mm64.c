                    ...
                    int pgnum)                      // num of mapping page
{
  ...
  /* Memset page table entries with dummy pattern for the address range */
  for (pgit = 0; pgit < pgnum; pgit++)
  {
    addr_t pgn = PAGING_PGN(addr) + pgit;
    if (pgn < PAGING64_MAX_PGN)
      caller->krnl->mm->pt[pgn] = pattern;
  }
...
{                                                   // no guarantee all given pages are mapped
  ...
  /* Map each frame to its virtual page in the page table */
  while (fpit != NULL && pgit < pgnum)
  {
    pgn = PAGING_PGN(addr) + pgit;
    /* Set the page table entry: present, not swapped, FPN = fpit->fpn */
    pte_set_fpn(caller, pgn, fpit->fpn);

    /* Track for FIFO page replacement */
...
addr_t alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
{
  ...
  *frm_lst = NULL;
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    {
      /* Not enough free frames - return error -3000 */
      return -3000;
    }
  }
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  /* Init multi-level page table directories */
  mm->pgd = (addr_t*)calloc(PAGING64_MAX_PGN, sizeof(addr_t));
  mm->p4d = (addr_t*)calloc(PAGING64_MAX_PGN, sizeof(addr_t));
  mm->pud = (addr_t*)calloc(PAGING64_MAX_PGN, sizeof(addr_t));
  mm->pmd = (addr_t*)calloc(PAGING64_MAX_PGN, sizeof(addr_t));
  mm->pt  = (addr_t*)calloc(PAGING64_MAX_PGN, sizeof(addr_t));
...
int print_pgtbl(struct pcb_t *caller, addr_t start, addr_t end)
{
  ...
  printf("print_pgtbl:\n");
  printf(" PDG=  %p PDG=  %p P4g=  %p PUD=  %p PMD=  %p\n",
         (void*)krnl->mm->pgd,
         (void*)krnl->mm->pgd,
         (void*)krnl->mm->p4d,
         (void*)krnl->mm->pud,
